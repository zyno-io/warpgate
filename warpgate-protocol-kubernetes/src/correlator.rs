...
use crate::server::auth::{
    EphemeralKubernetesIdentityCache, KubernetesAuthentication, access_denied,
    authorize_kubernetes_target, unauthorized,
};
...
pub struct AdmittedSession {
    ...
    pub upstream_certificate_cache: EphemeralKubernetesIdentityCache,
    ticket_grant: bool,
}
...
impl AdmittedSession {
    pub async fn verify_current(
        ...
        if warpgate_core::has_active_self_service_ticket(
            &services.db,
            user.id,
            self.approved.target().id,
        )
        .await?
        {
            Ok(())
        } else {
            Err(access_denied(&self.approved.target().name))
        }
    }
}
...
pub async fn correlated_authorization(
    ...
    loop {
        ...
        {
            Ok(resolved) => {
                let (resolved, ticket_grant, upstream_certificate_cache) = resolved.into_parts();
